...
public:
  ...
  vector<tuple<vector<Bomb>, BitField, Player, Player, BitField, vector<Feature>, StateQuality, Target>> get_action(vector<tuple<vector<Bomb>, BitField, Player, Player, BitField, vector<Feature>, StateQuality, Target>> states, int top_bombs, int top_features, bool only_first_bomb = false)
  {
      ...
      { // * just place bomb at x y
        ...
        for (int tick = 0; tick < K; tick++)
        {
          if (me_possible_pos.count() == 0)
            break;
          if (tick == 1 && only_first_bomb)  break;
          if (me.bombs > 0)
            for (int y = 0; y < H; y++)
            {
              for (int x = 0; x < W; x++)
  ...
  void prepare()
  {
    ...
    vector<tuple<vector<Bomb>, BitField, Player, Player, BitField, vector<Feature>, StateQuality, Target>> initial_states, post_initial_states, first_states, second_states, third_states, final_states;
    ...
    StateQuality sq;
    {
      first_states = get_action(initial_states, 4, 2);
    ...
    }
    {
      Target ntarget;
      StateQuality nsq;
      post_initial_states = get_action(initial_states, 1, 0, true);
      first_states = get_action(post_initial_states, 4, 2);
      second_states = get_action(first_states, 3, 2);
      final_states = get_action(second_states, 1, 0);
      if(final_states.size() > 0) {
        tie(ignore, ignore, ignore, ignore, ignore, ignore, nsq, ntarget) = final_states[0];
        if(nsq < sq) sq = nsq, target = ntarget
      }
    }

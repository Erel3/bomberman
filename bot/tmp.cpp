...
public:
  ...
  tuple<int, int> simulate_bombs(Player &me, Player &enemy, vector<bitset<W>> &me_possible_pos, vector<bitset<W>> &enemy_possible_pos, vector<bitset<W>> &bit_field, vector<Bomb> &bombs, vector<Feature> &features)
  {
    for (Bomb &bomb : bombs)
      bomb.timer--;
  ...
  void simulate_moves(Player &me, Player &enemy, vector<bitset<W>> &me_possible_pos, vector<bitset<W>> &enemy_possible_pos, vector<bitset<W>> &bit_field, vector<Bomb> &bombs, bool set_bomb = false)
  {
  ...
  tuple<bool, bool, int, int> check_safe_and_get_score(Player me, Player enemy, vector<bitset<W>> enemy_possible_pos, vector<bitset<W>> bit_field, vector<Bomb> bombs, vector<Feature> features)
  {
        ...
        if (me.bombs > 0)
          for (int y = 0; y < H; y++)
          {
            for (int x = 0; x < W; x++)
            {
              ...
              simulate_moves(me_bomb, enemy_bomb, me_possible_pos_bomb, enemy_possible_pos_bomb, bit_field, bombs_bomb, true);
              ...
              {
                tie(me_will_alive, enemy_will_alive, me_will_score, enemy_will_score) = check_safe_and_get_score(me_bomb, enemy_bomb, enemy_possible_pos_bomb, bit_field, bombs_bomb, features);
              }
              else
              {
                if (!is_calced[y][x])
  ...
  tuple<int, int> get_tick_score(Player me, Player enemy, vector<bitset<W>> bit_field, vector<Bomb> bombs, vector<Feature> features)
  {
    vector<bitset<W>> me_possible_pos = vector<bitset<W>>(H, bitset<W>());
  ...
  void prepare()
  {
    ...
    Target target;
    StateQuality sq;
    tie(ignore, ignore, ignore, ignore, ignore, ignore, sq, target) = get_action(states, 1, 0)[0];

...
public:
    ...
    while (recheck)
    {
      ...
      for (Bomb &bomb : bombs)
      {
        if (bomb.timer == 0 || destroyed_me.count(mp(bomb.x, bomb.y)) || destroyed_others.count(mp(bomb.x, bomb.y)))
        {
          ...
          if (bomb.owner_id == player.owner_id) player.bombs++;
        }
        else
        {
          new_bombs.pb(bomb);
  ...
  {
    {
      ...
      apply_move(bombs, field, me, PLAYER_BOMB);
      bool our_bomb_destroyed = false;
      bool bomb_hits_our_position = false;
      next_tick(bombs, field, me, our_bomb_destroyed, bomb_hits_our_position);
      int next_tick_with_bomb = get_bomb_restore_ticks(bombs, field, me);
      if (bomb_hits_our_position)
        return false;
      bool alive = get<3>(get_action(bombs, field, me.x, me.y, next_tick_with_bomb, true)) > 10;
      if (!alive)
        return false;
    }
    bool dead = true;
    // 1. bomb
    {
      ...
      Field field = this->field;
      vector<Bomb> bombs = this->bombs;
      // check order of applies!!
      if (apply_move(bombs, field, enemy, PLAYER_BOMB))
      {
        apply_move(bombs, field, me, PLAYER_BOMB);
        bool our_bomb_destroyed = false;
        bool bomb_hits_our_position = false;
        next_tick(bombs, field, enemy, our_bomb_destroyed, bomb_hits_our_position);
        int next_tick_with_bomb = get_bomb_restore_ticks(bombs, field, enemy);
        if (!bomb_hits_our_position)
        {
          dead &= get<3>(get_action(bombs, field, enemy.x, enemy.y, next_tick_with_bomb, true)) < 0;
        }
      }
    }
    // 2. stay
    {
      ...
      vector<Bomb> bombs = this->bombs;
      // check order of applies!!
      if (apply_move(bombs, field, enemy, PLAYER_STAY))
      {
        apply_move(bombs, field, me, PLAYER_BOMB);
        bool our_bomb_destroyed = false;
        bool bomb_hits_our_position = false;
    ...
    // 3. left
    {
      ...
      vector<Bomb> bombs = this->bombs;
      // check order of applies!!
      if (apply_move(bombs, field, enemy, PLAYER_LEFT))
      {
        apply_move(bombs, field, me, PLAYER_BOMB);
        bool our_bomb_destroyed = false;
        bool bomb_hits_our_position = false;
    ...
    // 4. right
    {
      ...
      vector<Bomb> bombs = this->bombs;
      // check order of applies!!
      if (apply_move(bombs, field, enemy, PLAYER_RIGHT))
      {
        apply_move(bombs, field, me, PLAYER_BOMB);
        bool our_bomb_destroyed = false;
        bool bomb_hits_our_position = false;
    ...
    // 5. up
    {
      ...
      vector<Bomb> bombs = this->bombs;
      // check order of applies!!
      if (apply_move(bombs, field, enemy, PLAYER_UP))
      {
        apply_move(bombs, field, me, PLAYER_BOMB);
        bool our_bomb_destroyed = false;
        bool bomb_hits_our_position = false;
    ...
    // 6. down
    {
      ...
      vector<Bomb> bombs = this->bombs;
      // check order of applies!!
      if (apply_move(bombs, field, enemy, PLAYER_DOWN))
      {
        apply_move(bombs, field, me, PLAYER_BOMB);
        bool our_bomb_destroyed = false;
        bool bomb_hits_our_position = false;
  ...
  void prepare()
  {
    ...
    if (enemy != nullptr && next_tick_with_bomb == 0)
    {
      int next_tick_with_bomb = get_bomb_restore_ticks(this->bombs, this->field, *this->enemy);
      // cerr << "bb " << next_tick_with_bomb << endl;
      bool dead = get<3>(get_action(this->bombs, this->field, this->enemy->x, this->enemy->y, next_tick_with_bomb)) < 0;
      // cerr << "dead " << get<3>(get_action(this->bombs, this->field, this->enemy->x, this->enemy->y, next_tick_with_bomb)) << endl;
      if (!dead && kill())
      {
        cerr << "----------------------------------------------------------------------------------------------------------------------------------------------DIE!" << endl;
